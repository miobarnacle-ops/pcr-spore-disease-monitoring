...
export default function InspectionConsole() {
  ...
  const forecastPanel = <>
    ...
    <Section title="模型对照 · 高斯烟羽 vs 物理引擎"><div className="button-grid two"><button className={showPlumeCompare ? "active-action" : ""} onClick={() => setShowPlumeCompare((old) => !old)}>{showPlumeCompare ? "地图渲染：高斯烟羽对照" : "地图渲染：物理引擎预测"}</button><button onClick={() => { if (!comparison) return showMessage("模型对照", "暂无对照数据：请先运行扩散预测（需 ≥2 个有效 PCR 观测）。"); showMessage("A/B 回代拟合诊断", <div className="table-wrap"><table><thead><tr><th>预报期</th><th>烟羽RMSE</th><th>引擎RMSE</th><th>烟羽偏差</th><th>引擎偏差</th></tr></thead><tbody>{comparison.slices.map((slice) => <tr key={slice.horizonHours}><td>{slice.horizonHours / 24}天</td><td>{slice.plumeRMSE.toFixed(3)}</td><td className={slice.eulerRMSE < slice.plumeRMSE ? "risk-low" : ""}>{slice.eulerRMSE.toFixed(3)}</td><td>{slice.plumeBias.toFixed(3)}</td><td>{slice.eulerBias.toFixed(3)}</td></tr>)}</tbody></table></div>); }}>查看量化对比</button></div><p className="hint">当前 PCR 点同时参与源估计，因此这里是回代拟合诊断，不代表独立的 3/5/7 天预测准确率。</p></Section>
