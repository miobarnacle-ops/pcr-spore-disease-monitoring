...
class InspectionSystem:
    ...
    def _build_interface(self):
        ...
        self.main_canvas.bind("<Button-3>", self._on_canvas_right_click)
    ...
    def _on_canvas_mouse_move(self, event):
        ...
        if current_tab == 0 and self.field_polygon:
            meter_x = (event.x - self.reference_x_px) / \
                self.scale_px_per_meter if self.scale_px_per_meter > 0 else 0
            meter_y = (event.y - self.reference_y_px) / \
                self.scale_px_per_meter if self.scale_px_per_meter > 0 else 0
            self.coordinate_label.config(
                text=f"px({event.x},{event.y})  m({meter_x:.1f},{meter_y:.1f})")
        elif current_tab >= 1 and self.sampling_points:
            self.coordinate_label.config(text=f"px({event.x},{event.y})")
        else:
    ...
    def _on_canvas_left_click(self, event):
        ...
        if not self.is_drawing_polygon:
            return
        self.field_polygon.append((event.x, event.y))
        self.undo_stack.append((event.x, event.y))
        ...
        if len(self.field_polygon) > 1:
            prev_x, prev_y = self.field_polygon[-2]
    ...
    def _undo_vertex(self):
        ...
        # 重绘剩余顶点
        for i, (vx, vy) in enumerate(self.field_polygon):
            self.main_canvas.create_oval(
                vx - 3, vy - 3, vx + 3, vy + 3,
                fill="#ffffff", outline=self.color_info, width=1)
            if i > 0:
                px, py = self.field_polygon[i - 1]
                color = self.color_info if i == 1 else "#ffffff"
                line_width = 3 if i == 1 else 1.5
                self.main_canvas.create_line(
                    px, py, vx, vy, fill=color, width=line_width)
        self.status_variable.set(
    ...
    def _draw_planning_view(self):
        ...
        if len(polygon) >= 2:
            for i in range(len(polygon)):
                ...
                self.main_canvas.create_line(
                    x1, y1, x2, y2, fill=edge_color, width=edge_width)
        ...
        for round_index, (nodes, tour) in enumerate(
            ...
            for local_idx, node in enumerate(nodes):
                ...
                self.main_canvas.create_oval(
                    nx - 2.5, ny - 2.5, nx + 2.5, ny + 2.5,
                    fill="#ffffff", outline="")
                label_text = f"R{round_index + 1}-P{local_idx + 1}"
                self.main_canvas.create_text(
                    nx + 11, ny - 17, text=label_text,
    ...
    def _draw_masked_heatmap(self, canvas_w, canvas_h, measured, polygon):
        # 细粒度渲染 + 平滑上采样 = 消除块状边缘
        coarse_step = max(6, min(canvas_w, canvas_h) // 80)
        fine_step = max(2, coarse_step // 3)
        render_w = (canvas_w + coarse_step - 1) // coarse_step
        render_h = (canvas_h + coarse_step - 1) // coarse_step
        radius_limit = (max(canvas_w, canvas_h) * 0.5) ** 2
        raw_small = Image.new("RGBA", (render_w, render_h), (0, 0, 0, 0))
        draw_handle = ImageDraw.Draw(raw_small)
        measured_pts = [
            (p['real_xy'][0], p['real_xy'][1], p['turbidity'])
            for p in measured]
        if not measured_pts:
            return
        ...
        raw_full = raw_small.resize(
            (canvas_w, canvas_h), Image.LANCZOS)
        mask = Image.new("L", (canvas_w, canvas_h), 0)
        ImageDraw.Draw(mask).polygon(
            [(int(x), int(y)) for x, y in polygon], fill=255)
        clipped = Image.composite(
            raw_full, Image.new("RGBA", (canvas_w, canvas_h), (0, 0, 0, 0)), mask)
    ...
    def _draw_sensing_circles(self, canvas_w, canvas_h):
        ...
        radius_px = self.coverage_radius_meters * scale_px_per_m
        overlay = Image.new("RGBA", (canvas_w, canvas_h), (0, 0, 0, 0))
        draw_handle = ImageDraw.Draw(overlay)
    ...
    def _sensing_canvas_click(self, event):
        ...
        for i, point in enumerate(self.sampling_points):
            d2 = (event.x - point['real_xy'][0]) ** 2 + \
                 (event.y - point['real_xy'][1]) ** 2
            if d2 < best_distance:
    ...
    def _pcr_import_csv(self):
        ...
        for row in rows:
            ...
            if matched is None:
                if not self.sampling_points:
                    try:
                        x_m = float(row[1]); y_m = float(row[2])
                        ridge = int(row[3]) if len(row) > 3 else 0
                    ...
                    matched = self.sampling_points[-1]
                    imported_count += 1
                else:
                    continue
            if len(row) >= 5 and row[4]:
                try:
                    matched['turbidity'] = float(row[4])
                except ValueError:
                    pass
    ...
    def _draw_pcr_view(self):
        ...
        pcr_valid = [p for p in self.sampling_points
                     if p.get('pcr_conc') is not None
                     and p.get('pcr_qual') == 'valid']
        coarse_pts = [p for p in self.sampling_points
                      if p.get('turbidity') is not None]
        if HAS_PILLOW and len(pcr_valid) >= 2:
            self._render_fusion_heatmap(
                canvas_w, canvas_h, pcr_valid, coarse_pts, polygon)
        for i, point in enumerate(self.sampling_points):
            px, py = point['real_xy']
            ...
            short_label = point['point_id'].replace(
                'R', '').replace('-P', '.')
            self.main_canvas.create_text(
                px + 11, py - 10, text=short_label,
                fill=outline_color, font=("Consolas", 7, "bold"), anchor="w")
        if self.show_legend and self.legend_checkbox_var.get():
            self._draw_pcr_legend(canvas_w, canvas_h)

    def _render_fusion_heatmap(self, canvas_w, canvas_h,
                                pcr_valid, coarse_pts, polygon):
        """PCR浓度直接IDW插值: 在log10空间做加权平均保证渐变自然"""
        coarse_step = max(5, min(canvas_w, canvas_h) // 90)
        render_w = max(1, (canvas_w + coarse_step - 1) // coarse_step)
        render_h = max(1, (canvas_h + coarse_step - 1) // coarse_step)
        radius_limit = (max(canvas_w, canvas_h) * 0.5) ** 2
        raw_small = Image.new("RGBA", (render_w, render_h), (0, 0, 0, 0))
        draw_handle = ImageDraw.Draw(raw_small)
        # 使用log10(PCR浓度)作为IDW值, 避免量纲差异问题
        pcr_log10 = [
            (p['real_xy'][0], p['real_xy'][1],
             math.log10(max(1, p['pcr_conc'])))
            for p in pcr_valid if p.get('pcr_conc', 0) > 0]
        if not pcr_log10:
            return
        # 确定log10值域用于归一化到色标
        log10_values = [v[2] for v in pcr_log10]
        log10_min = min(log10_values) if log10_values else -1
        log10_max = max(log10_values) if log10_values else 2
        log10_range = max(log10_max - log10_min, 0.5)
        for rx in range(render_w):
            gx = rx * coarse_step
            for ry in range(render_h):
                gy = ry * coarse_step
                if not point_in_polygon(gx, gy, polygon):
                    continue
                # IDW of log10(PCR浓度)
                ws, vs = 0.0, 0.0
