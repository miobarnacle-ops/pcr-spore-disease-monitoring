...
class InspectionSystem :
    ...
    def _simulate_turbidity_reading (self ,sampling_point ):
        ...
        if mode =="uniform":
            ...
            cx_m =(centroid [0 ]-self .reference_x_px )/self .scale_px_per_meter if self .scale_px_per_meter >0 else 0 
            cy_m =(centroid [1 ]-self .reference_y_px )/self .scale_px_per_meter if self .scale_px_per_meter >0 else 0 
            dist =math .hypot (
    ...
    def _sensing_canvas_click (self ,event ):
        ...
        img_x ,img_y =self .canvas_to_image (event .x ,event .y )
        for i ,point in enumerate (self .sampling_points ):
            d2 =(img_x -point ['real_xy'][0 ])**2 +(img_y -point ['real_xy'][1 ])**2 
            if d2 <best_distance :
