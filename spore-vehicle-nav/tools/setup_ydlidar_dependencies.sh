...
# The ROS 2 driver source is part of this repository.  Keep the fallback clone
# for a source-only checkout, but do not ask git clone to overwrite the synced
# non-empty directory.
if [[ -d "${driver_dir}" && ! -d "${driver_dir}/.git" ]]; then
  echo "Using repository-synced YDLIDAR ROS 2 driver at ${driver_dir}."
else
