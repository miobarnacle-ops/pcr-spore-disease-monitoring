...
# ------------------------------------------------------------
log "[1] 环境自检"
/usr/bin/python3 -c "import serial" >/dev/null 2>&1 && log "  pyserial OK" || { log "  ✗ pyserial 缺失，尝试修复"; echo '[REDACTED_SUDO_PASSWORD]' | sudo -S apt-get install -y -qq python3-serial >>"$LOGDIR/main.log" 2>&1 && log "  ✓ 已修复" || { log "  ✗ 修复失败，中止"; exit 1; }; }
command -v screen >/dev/null || echo '[REDACTED_SUDO_PASSWORD]' | sudo -S apt-get install -y -qq screen >>"$LOGDIR/main.log" 2>&1
